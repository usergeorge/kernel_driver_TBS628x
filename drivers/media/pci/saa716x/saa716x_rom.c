				 ...
				 u32 *offset)
{
	...
	if (rom_header->header_size != sizeof (struct saa716x_romhdr)) {
		dprintk(SAA716x_ERROR, 1,
			"ERROR: Header size mismatch! Read size=%lu bytes, Expected=%d",
				...
				u32 *offset)
{
	...
	if (header.size != sizeof (struct saa716x_decoder_hdr)) {
		dprintk(SAA716x_ERROR, 1,
			"ERROR: Header size mismatch! Read size=%d bytes, Expected=%lu",
			header.size,
			       ...
			       u32 *offset)
{
	...
	if (device->struct_size != sizeof (struct saa716x_devinfo)) {
		dprintk(SAA716x_ERROR, 1, "ERROR: Device size mismatch! Read=%d bytes, expected=%lu bytes",
		device->struct_size,
